...
def save_checkpoint(state, epoch, filename='checkpoint.pth.tar'):
    ...
    weight_dir = os.path.join(root_dir, 'weight_noise7_trans_rotate')

    if not os.path.exists(weight_dir):
        os.mkdir(weight_dir)

    filename = 'epoch_'+str(epoch)+'_checkpoint.pth'
    file_path = os.path.join(weight_dir, filename)
    torch.save(state, file_path)

    if state['is_best']:
        src = file_path
        dst = os.path.join(weight_dir, 'best_model.pth')
        shutil.copyfile(src, dst)
    print('EAST <==> Save weight - epoch {} <==> Done'.format(epoch))


def save_checkpoint_step(state, epoch, step, filename='checkpoint.pth.tar'):
    """[summary]
    ...
    filename = 'epoch_'+str(epoch)+'_step_'+str(step)+'_checkpoint.pth'
    file_path = os.path.join(weight_dir, filename)
    torch.save(state, file_path)
