...
# sys.path.append(r"/home/fujiayun/PycharmProjects/EAST/geo_map_cython_lib")
# import gen_geo_map
...
def check_and_validate_polys(polys, xxx_todo_changeme):
    ...
    validated_polys = []
    validated_tags = []
    #####I comment it,because dataset I made is correct,don't need check
...
def crop_area(im, polys, crop_background=False, max_tries=20, vis = False, img_name = None):
    '''
...
def restore_rectangle_rbox(origin, geometry):
    if origin.shape[0] > 0:
        new_p0 = origin[:, :]-geometry[:, :2]
        new_p0 = np.expand_dims(new_p0, 1)
        new_p1 = origin[:, :] - geometry[:, 2:4]
        new_p1 = np.expand_dims(new_p1, 1)
        new_p2 = origin[:, :] - geometry[:, 4:6]
        new_p2 = np.expand_dims(new_p2, 1)
        new_p3 = origin[:, :] - geometry[:, 6:]
        new_p3 = np.expand_dims(new_p3, 1)
        new_p =  np.concatenate([new_p0, new_p1, new_p2, new_p3], axis=1)
    else:
...
def gen_quad_geo_map(geo_map, xy_in_poly, poly):
    ...
    for i in range(num_pt):
        y = xy_in_poly[i, 0]
        x = xy_in_poly[i, 1]
        geo_map[y, x, 0] = x - p0[0]
        geo_map[y, x, 1] = y - p0[1]
        geo_map[y, x, 2] = x - p1[0]
        geo_map[y, x, 3] = y - p1[1]
        geo_map[y, x, 4] = x - p2[0]
        geo_map[y, x, 5] = y - p2[1]
        geo_map[y, x, 6] = x - p3[0]
        geo_map[y, x, 7] = y - p3[1]
...
def generate_rbox(im_size, polys):
    ...
    geo_map = np.zeros((int(h/4), int(w/4), 8), dtype=np.float32)
    # mask used during traning, to ignore some hard areas
    training_mask = np.ones((int(h/4), int(w/4)), dtype=np.uint8)
    for poly_idx, poly in enumerate(polys):
        ...
        xy_in_poly = np.argwhere(score_map == 1)
        # # if geometry == 'RBOX':
        # # 对任意两个顶点的组合生成一个平行四边形
        ...
        # gen_quad_geo_map(geo_map, xy_in_poly, poly)
        # geometry == 'QUAD':
        for y, x in xy_in_poly:
            geo_map[y, x, 0] = x*4 - poly[0][0]
            geo_map[y, x, 1] = y*4 - poly[0][1]
            geo_map[y, x, 2] = x*4 - poly[1][0]
            geo_map[y, x, 3] = y*4 - poly[1][1]
            geo_map[y, x, 4] = x*4 - poly[2][0]
            geo_map[y, x, 5] = y*4 - poly[2][1]
            geo_map[y, x, 6] = x*4 - poly[3][0]
            geo_map[y, x, 7] = y*4 - poly[3][1]
    ###sum up
...
def image_label(txt_root, 
    ...
    try:
        ...
        if np.random.rand() < background_ratio:
            ...
            score_map = np.zeros((int(input_size/4), int(input_size/4)), dtype=np.uint8)
            geo_map_channels = 8
            geo_map = np.zeros((int(input_size/4), int(input_size/4), geo_map_channels), dtype=np.float32)
            training_mask = np.ones((int(input_size/4), int(input_size/4)), dtype=np.uint8)
        else:
            ...
            if len(text_polys) == 0: #for some reason , gt contain no polys, have to return black
                ...
                score_maps = score_map[:, :, np.newaxis].astype(np.float32)
                geo_maps = geo_map[:, :, :].astype(np.float32)
                training_masks = training_mask[:, :, np.newaxis].astype(np.float32)
            ...
            new_h, new_w, _ = im.shape
            #print('done3')
            score_map, geo_map, training_mask = generate_rbox((new_h, new_w), text_polys)
            #print('done4')
...
class custom_dset(data.Dataset):
    def __init__(self, img_root, txt_root, vis = False):
        ...
        for i in range(len(self.img_path_list)):
            img_id = []
            img_id.append(os.path.basename(self.img_path_list[i]).strip('.png'))
            img_id.append(os.path.basename(self.txt_path_list[i]).strip('.txt'))
            img_id.append(self.img_name_list[i].strip('.png'))
    ...
    def __getitem__(self, index):
        ...
        while status:
            img, score_map, geo_map, training_mask = image_label(self.txt_root,
                ...
                random_scale = np.array([0.5, 1.0, 2.0, 3.0]), background_ratio = 1./10.)
